...
### 텍스트 메시지
```
수신 → 권한 체크 → 세션 Lock → ensure_session → typing 주기 전송(4초) → claude -p → Markdown→HTML 변환 → 분할 전송
```

### 파일 (사진/문서)
```
수신 → 권한 체크 → 세션 Lock → workspace에 다운로드 → 캡션 + 파일경로를 Claude에 전달 → 응답 전송
```
