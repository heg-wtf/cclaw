"""Backup abyss home directory to an encrypted zip file."""
...
EXCLUDE_FILENAMES = {"abyss.pid"}
EXCLUDE_DIRECTORY_NAMES = {"__pycache__"}


def generate_backup_filename() -> str:
    """Generate a backup filename in YYMMDD-abyss.zip format."""
    return datetime.now().strftime("%y%m%d") + "-abyss.zip"
...
def create_encrypted_backup(
    ...
    """Create an AES-256 encrypted zip backup of the abyss home directory.

    Args:
        output_path: Path where the zip file will be written.
        password: Encryption password.
        home_directory: The abyss home directory to back up.
